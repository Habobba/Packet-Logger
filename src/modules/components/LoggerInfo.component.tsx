import React from "react"

export default function LoggerInfo(): JSX.Element {
    const contributors = [
        { name: 'nicollassilva', link: 'https://github.com/nicollassilva' },
        { name: 'henriqFerreira', link: 'https://github.com/henriqFerreira' }
    ]
    
    return (
        <div className="hb-flex-1 hb-flex hb-flex-col hb-gap-2 hb-p-2 hb-m-2 hb-rounded-sm hb-bg-neutral-900">
            <div className="hb-flex hb-items-center hb-gap-2 hb-bg-neutral-950 hb-rounded-[4px] hb-p-2">
                <img
                    src="data:image/webp;base64,"
                    alt="Habobba Logo"
                    className="hb-w-16 hb-h-16"
                />
                <div>
                    <h2 className="hb-text-xl hb-font-bold hb-text-white">Habobba 1.0.0</h2>
                    <p className="hb-text-gray-400 hb-text-sm">
                        Manipulador de pacotes de Habbo Piratas.
                    </p>
                </div>
            </div>
            <div className="hb-flex hb-flex-col hb-gap-2">
                <h3 className="hb-text-lg hb-text-white">Criado por:</h3>
                <a target="_blank" href='https://github.com/Cor3Duo' className="hb-text-gray-400 hover:hb-text-blue-400 hover:hb-underline hb-text-sm hb-pl-3">CoreDuo</a>
            </div>
            <div className="hb-flex hb-flex-col hb-gap-2">
                <h3 className="hb-text-lg hb-text-white">Contribuidores:</h3>
                <div className="hb-grid hb-grid-cols-2 hb-gap-x-4 hb-gap-y-2 hb-pl-3">
                    {contributors.map((contributor, index) => (
                        <a target="_blank" key={index} href={contributor.link} className="hb-text-gray-400 hover:hb-text-blue-400 hover:hb-underline hb-text-sm">
                            {contributor.name}
                        </a>
                    ))}
                </div>
            </div>
            <div className="hb-flex hb-flex-col hb-gap-2">
                <h3 className="hb-text-lg hb-text-white">Links:</h3>
                <a href="https://discord.gg/9vWNdR2Dd8" className="hb-text-blue-400 hover:hb-underline hb-text-sm hb-pl-3">
                    <i className="fa-brands fa-discord hb-w-5"></i> Discord
                </a>
                <a href="https://github.com/Habobba/Packet-Logger" className="hb-text-blue-400 hover:hb-underline hb-text-sm hb-pl-3">
                    <i className="fa-brands fa-github hb-w-5"></i> Github - Habobba Logger
                </a>
            </div>
        </div>
    )
}